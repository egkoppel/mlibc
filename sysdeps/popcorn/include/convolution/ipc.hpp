...
namespace convolution::ipc {
	...
	class Sender {
	public:
		explicit Sender(const char *address) : m_buffer(convolution_ipc_open(address)) {
			// TODO: if (this->handle <= 0) throw std::runtime_error("Failed to register IPC receiver");
		}
		~Sender() { convolution_ipc_close(this->m_buffer); }

		void *buffer() { return this->m_buffer; }
		const void *buffer() const { return this->m_buffer; }

		void notify() {
			if (convolution_ipc_notify(this->m_buffer) != 0) {
				// TODO: error?
			}
		}
